...
for k = 1:nzp
    ...
    eos.Vartheta1_alt(k) = theta_variance(state.fluid(1).eta(k),state.fluid(1).q(k),...
                                          etastd*etastd,qstd*qstd,...
                                          eos.Varql1(k),eos.Covarqleta1(k),eos.Covarqlq1(k),...
                                          constants.therm,pbar,constants.phys.p00);
    ...
    % Potential temperature variance
    eos.Vartheta2_alt(k) = theta_variance(state.fluid(2).eta(k),state.fluid(2).q(k),...
                                          etastd*etastd,qstd*qstd,...
                                          eos.Varql2(k),eos.Covarqleta2(k),eos.Covarqlq2(k),...
                                          constants.therm,pbar,constants.phys.p00);
                
    eos.pbar(k) = pbar;
    
    eos.theta1_alt(k) = potential_temp(state.fluid(1).Tw(k),pbar,constants.phys,constants.therm);
    eos.theta2_alt(k) = potential_temp(state.fluid(2).Tw(k),pbar,constants.phys,constants.therm);
    eos.thetal1(k) = eta2thetal(state.fluid(1).eta(k), ...
                                state.fluid(1).q(k), ...
                                state.fluid(1).Tw(k), ...
                                constants.therm, constants.phys.p00);
    eos.thetal2(k) = eta2thetal(state.fluid(2).eta(k), ...
                                state.fluid(2).q(k), ...
                                state.fluid(2).Tw(k), ...
                                constants.therm, constants.phys.p00);
    eos.theta1(k) = eos.thetal1(k)/(1 - (constants.therm.L0*eos.ql1(k))...
                                       /(constants.therm.Cpd*state.fluid(1).Tw(k)));
    eos.theta2(k) = eos.thetal2(k)/(1 - (constants.therm.L0*eos.ql2(k))...
                                       /(constants.therm.Cpd*state.fluid(2).Tw(k)));
    ...
    eos.Vartheta1(k) = eos.Varrhow1(k)*(eos.theta1(k)/eos.rhow1(k))^2;
    eos.Vartheta2(k) = eos.Varrhow2(k)*(eos.theta2(k)/eos.rhow2(k))^2;
