...
for k = 1:nzp
    ...
    eos.theta1_alta(k) = potential_temp(state.fluid(1).Tw(k),pbar,constants.phys,constants.therm);
    eos.theta2_alta(k) = potential_temp(state.fluid(2).Tw(k),pbar,constants.phys,constants.therm);
    ...
    eos.theta2(k) = eta2thetal( state.fluid(2).eta(k), ...
                                ...
                                state.fluid(2).Tw(k), ...
                                constants.therm, constants.phys.p00);
    eos.theta1_altb(k) = eos.theta1(k)/(1 - (constants.therm.L0*eos.ql1(k))...
                                           /(constants.therm.Cpd*state.fluid(1).Tw(k)));
    eos.theta2_altb(k) = eos.theta2(k)/(1 - (constants.therm.L0*eos.ql2(k))...
